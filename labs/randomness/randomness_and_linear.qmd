...
When we look at the residuals, really this is just extra variance. It's misleading to call it "error" or "noise" although these are the common ways to refer to $\hat{\varepsilon}_i$.

Let's consider two types of sources of extra variance that are truly not "error" or "noise".
...
Besides our formula above for estimating sampling variability of various quantities, which all relied on $\hat{\sigma}^2$, we can also use bootstrapping to estimate variances. Let's show an example where one of the assumptions is not true, and compare the standard estimators to bootstrap estimators.
...
b <- c(5, 2, 2)
y_linear <- X %*% b
extra <- rnorm(n, sd = y_linear/5) # varies with the linear part of y
y <- y_linear + extra 
plot(y_linear, y, cex=.5)
```
